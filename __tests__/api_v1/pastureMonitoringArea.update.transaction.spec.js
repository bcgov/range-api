...
const seedData = {
  ...
  monitoringArea: { id: 1, plant_community_id: 1, name: 'Original Area' },
  monitoringAreaPurpose: { id: 1, monitoring_area_id: 1, purpose_type_id: 1 },
};

describe('PlanPastureController.updateMonitoringArea() transaction', () => {
  beforeAll(async () => {
  ...
  test('updates a monitoring area and its purposes on valid PUT', async () => {
    const app = await createApp();

    await request(app)
      .put(`${baseUrl}/1/pasture/1/plant-community/1/monitoring-area/1`)
      .send({ name: 'Updated Area', purposeTypeIds: [2] })
      .expect(200)
      .expect((res) => {
  ...
  test('returns 500 and rolls back all writes on FK constraint violation', async () => {
    ...
    const areas = await db.selectFrom('monitoring_area').selectAll().execute();
    expect(areas).toHaveLength(1);
    expect(areas[0].name).toBe('Original Area');

    const purposes = await db.selectFrom('monitoring_area_purpose').selectAll().execute();
    expect(purposes).toHaveLength(1);
    expect(purposes[0].purpose_type_id).toBe(1);
  });
});
