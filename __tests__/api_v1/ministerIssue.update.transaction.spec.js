...
const seedData = {
  ...
  ministerIssue: { id: 1, plan_id: 1, issue_type_id: 1, detail: 'Existing Issue' },
  ministerIssuePasture: { minister_issue_id: 1, pasture_id: 1 },
};

describe('PlanMinisterIssueController.update() transaction', () => {
  beforeAll(async () => {
  ...
  beforeEach(async () => {
    ...
    await db('minister_issue').insert([seedData.ministerIssue]);
    await db('minister_issue_pasture').insert([seedData.ministerIssuePasture]);
  });

  test('updates a minister issue and pasture links on valid PUT', async () => {
    const app = await createApp();

    await request(app)
      .put(`${baseUrl}/1/issue/1`)
      .send({ issueTypeId: 2, detail: 'Updated Issue', pastures: [2] })
      .expect(200)
      .expect((res) => {
        expect(res.body.id).toBe(1);
        expect(res.body.detail).toBe('Updated Issue');
      });

    const issues = await db.selectFrom('minister_issue').selectAll().execute();
    expect(issues).toHaveLength(1);
    expect(issues[0].issue_type_id).toBe(2);
    expect(issues[0].detail).toBe('Updated Issue');

    const pastureLinks = await db.selectFrom('minister_issue_pasture').selectAll().execute();
    expect(pastureLinks).toHaveLength(1);
    expect(pastureLinks[0].pasture_id).toBe(2);
    expect(pastureLinks[0].minister_issue_id).toBe(1);
  });
  ...
  test('returns 500 and rolls back on FK constraint violation', async () => {
    ...
    await request(app)
      .put(`${baseUrl}/1/issue/1`)
      .send({ issueTypeId: 99999, detail: 'Should fail', pastures: [2] })
      .expect(500);

    const issues = await db.selectFrom('minister_issue').selectAll().execute();
    expect(issues).toHaveLength(1);
    expect(issues[0].issue_type_id).toBe(1);
    expect(issues[0].detail).toBe('Existing Issue');

    const pastureLinks = await db.selectFrom('minister_issue_pasture').selectAll().execute();
    expect(pastureLinks).toHaveLength(1);
    expect(pastureLinks[0].pasture_id).toBe(1);
  });
});
