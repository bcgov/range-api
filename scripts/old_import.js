...
const updateAgreement = async (data) => {
  for (var i = 0; i < data.length; i++) {
    ...
    if (
      ...
      !record.district_admin_zone ||
      !record.file_type_code
    ) {
    ...
    try {
      ...
      if (!zone) {
        throw new Error(
          `No zone with code ${record.district_admin_zone || 'NULL'}`,
        );
      }
      ...
      if (!atype) {
        throw new Error(
          `No AgreementType with code ${record.file_type_code || 'NULL'}`,
        );
      }

      const exemption = await AgreementExemptionStatus.findOne(db, {
        code: 'N',
      }); // Not Exempt
      ...
      console.log(
        `Error with message = ${error.message}, ID ${record.forest_file_id}`,
      );
    }
  }
};
...
const updateDistrict = async (data) => {
  for (var i = 0; i < data.length; i++) {
    ...
    try {
      ...
      console.log(
        `Error with message = ${error.message}, District ID ${record.org_unit_code}`,
      );
    }
  }
};
...
const updateZone = async (data) => {
  for (var i = 0; i < data.length; i++) {
    ...
    try {
      ...
      console.log(
        `Can not update Zone. Error = ${error.message}, Zone = ${record.district_admin_zone}, ID ${record.forest_file_id}`,
      );
    }
  }
};
...
const updateUsage = async (data) => {
  for (var i = 0; i < data.length; i++) {
    ...
    if (!isValidRecord(record) || !record.calendar_year) {
      console.log(
        `Skipping record with ID ${record.forest_file_id}, Year = ${record.calendar_year}`,
      );
      continue;
    }
...
const updateClient = async (data) => {
  for (var i = 0; i < data.length; i++) {
    ...
    if (!isValidRecord(record) || !record.client_locn_code) {
      console.log(
        `Skipping record with ID ${record.client_number}, Agreement ID = ${record.forest_file_id}`,
      );
      continue;
    }
    ...
    try {
      ...
      if (!client) {
        client = await Client.create(db, {
          ...
          startDate: record.licensee_start_date
            ? parseDate(record.licensee_start_date)
            : null,
        });
        // const agreement = await Agreement.findById(db, record.forest_file_id);
        // if (agreement) {
...
const updateUser = async (data) => {
  for (var i = 0; i < data.length; i++) {
    ...
    if (!username || !zoneCode) {
      console.log(
        `Skipping record with CODE ${zoneCode}, username = ${username} row = ${i}`,
      );
      continue;
    }
    ...
    try {
      ...
      console.log(
        `Imported User ID = ${user.id}, username  = ${user.username} and updated Zone ${zoneCode} owner.`,
      );
    } catch (error) {
      console.log(
        `Can not update User with username ${username}. error = ${error.message}`,
      );
    }
  }
};
