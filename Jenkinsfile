      ...
      script {
        ...
        try {
          ...
          sh returnStdout: true,
             script: "./gradlew sonarqube -Dproject.settings=../sonar-project.properties -Dsonar.host.url=${SONARQUBE_URL} -Dsonar.verbose=true --stacktrace --info -Dsonar.branch=${GIT_BRANCH_NAME}"  
          }
        } catch (error) {
          def attachment = [:]
          attachment.fallback = 'See build log for more details'
