...
gulp.task('copy-config', () =>
  gulp.src('src/config/*.json').pipe(gulp.dest('build/src/config')),
);
...
gulp.task('copy-tools', () =>
  gulp
    .src(['wkhtmltopdf-amd64-0.12.4/**/*'], { dot: false })
    .pipe(gulp.dest('build/src/wkhtmltopdf-amd64-0.12.4')),
);

gulp.task('copy-templates', () =>
  gulp.src(['templates/*'], { dot: false }).pipe(gulp.dest('build/templates')),
);
