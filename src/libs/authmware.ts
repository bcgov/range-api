...
import { logger, errorWithCode } from './bcgov-shim.js';
import passport from 'passport';
import { ExtractJwt, Strategy as JwtStrategy } from 'passport-jwt';
import jwksRsa from 'jwks-rsa';
import config from '../config/index.js';
import DataManager from './db2/index.js';
import { SSO_ROLE_MAP } from '../constants.js';
import UserPermissions from './db2/model/userPermissions.js';
import ClientAgreement from './db2/model/ClientAgreement.js';
      ...
      try {
        ...
        if (!user) {
          ...
          if (computedUsername !== null) {
            ...
            if (user == null) {
              logger.debug(`user ${computedUsername} not found in database, not migrating`);
            } else {
              logger.debug(
