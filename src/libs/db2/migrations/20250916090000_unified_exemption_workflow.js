...
exports.up = async function (knex) {
  ...
  `);
  await knex.raw(`
    INSERT INTO agreement_exemption_status (code, description, active)
    VALUES
      ('NO_EXEMPTION', 'No Exemption', TRUE),
      ('EXEMPTED', 'Exempted', TRUE),
      ('EXPIRED', 'Expired', TRUE),
      ('EXEMPTION_SCHEDULED', 'Exemption Scheduled', TRUE)
  `);

  // 2. Add exemption_status column to agreement table
  await knex.raw(`ALTER TABLE agreement ADD COLUMN exemption_status VARCHAR(32) NOT NULL DEFAULT 'NO_EXEMPTION';`);
  await knex.raw(
  ...
  await knex.raw(`UPDATE agreement SET exemption_status = 'NO_EXEMPTION';`);
