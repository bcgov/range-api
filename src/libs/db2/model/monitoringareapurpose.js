...
export default class MonitoringAreaPurpose extends Model {
  constructor(data, db = undefined) {
    ...
    Object.keys(data).forEach((key) => {
      if (MonitoringAreaPurpose.fields.indexOf(`${MonitoringAreaPurpose.table}.${key}`) > -1) {
        obj[key] = data[key];
      }
    });
  ...
  static get fields() {
    ...
    return ['id', 'purpose_type_id', 'monitoring_area_id', 'canonical_id'].map((field) => `${this.table}.${field}`);
  }
  ...
  static async findWithType(db, where) {
    const myFields = [
      ...
      ...MonitoringAreaPurposeType.fields.map((f) => `${f} AS ${f.replace('.', '_')}`),
    ];
    ...
    const results = await db
      ...
      .leftJoin('ref_monitoring_area_purpose_type', {
        'monitoring_area_purpose.purpose_type_id': 'ref_monitoring_area_purpose_type.id',
      })
      .where(where)
      .orderBy('id', 'asc');
