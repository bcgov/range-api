...
export default class ExemptionAttachment extends Model {
   ...
   */
  static async findByExemptionId(db, exemptionId) {
    if (!db || !exemptionId) {
      return [];
    }
    ...
    return results.map((row) => {
      const { given_name, family_name, ...attachmentData } = row;
      const user = given_name || family_name ? { givenName: given_name, familyName: family_name } : null;
      const finalRow = {
        ...attachmentData,
        user,
      };
      return new ExemptionAttachment(finalRow, db);
    });
  }
}
