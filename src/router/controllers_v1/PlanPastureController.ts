...
import { PLANT_COMMUNITY_CRITERIA, PURPOSE_OF_ACTION } from '../../constants.js';
...
export default class PlanPastureController {
  ...
  static async getPasturesForDistrict(req, res) {
    ...
    const pastures = await db
      ...
      .where('ref_district.id', '=', Number(districtId))
      .orderBy('pasture.id', 'asc')
      .orderBy('pasture.name', 'asc')
      .execute();
    const uniquePastures = await Promise.all(pastures.map((p) => Pasture.findById(db, p.id)));
    await Promise.all(uniquePastures.flatMap((p) => [p.fetchPlantCommunities(db, { pasture_id: p.id })]));
  ...
  static async updatePlantCommunityAction(req, res) {
    ...
    checkRequiredFields(['planId', 'pastureId', 'communityId', 'actionId'], 'params', req);
  ...
  static async updateIndicatorPlant(req, res) {
    ...
    checkRequiredFields(['planId', 'pastureId', 'communityId', 'plantId'], 'params', req);
  ...
  static async updateMonitoringArea(req, res) {
    ...
    checkRequiredFields(['planId', 'pastureId', 'communityId', 'areaId'], 'params', req);
