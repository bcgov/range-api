...
import { isNumeric, checkRequiredFields } from '../../libs/utils.js';
import DataManager from '../../libs/db2/index.js';
import config from '../../config/index.js';
import { dayjs as moment } from '../../libs/bcgov-shim.js';
import { AGREEMENT_EXEMPTION_STATUS, EXEMPTION_STATUS, PLAN_STATUS, SSO_ROLE_MAP } from '../../constants.js';
...
export default class PlanStatusController {
  ...
  static async update(req, res) {
    ...
    try {
      await db.transaction().execute(async (trx) => {
        ...
        status = planStatuses.find((s) => s.id === statusId);
        if (!status) {
  ...
  static async getLatestLegalVersion(planId, legalStatusesToSearch) {
    const latestLegalVersion = await db
      .selectFrom('plan_snapshot')
      .selectAll()
      .where('status_id', 'in', legalStatusesToSearch || Plan.legalStatuses)
