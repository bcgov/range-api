...
import PlanFile from '../../libs/db2/model/PlanFile.js';
...
export default class PlanExtensionController {
  ...
  static async createReplacementPlan(req, res) {
    ...
    return res
      .status(200)
      .json({ replacementPlan: { id: newPlanId } })
      .end();
  }
  ...
  static async updateGrazingScheduleEntriesYear(trx, grazingScheduleId, targetYear) {
    ...
    for (const entry of entries) {
      ...
      await trx
        .updateTable('grazing_schedule_entry')
        .where('id', '=', entry.id)
        .set({ date_in: updatedDateIn, date_out: updatedDateOut })
        .execute();
    }
  }
  ...
  static async copyPlan(req, res) {
    ...
    let newPlan;
    await db.transaction().execute(async (trx) => {
      newPlan = await PlanController.duplicatePlan(
        trx,
        planRow,
        {
